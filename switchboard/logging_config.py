...
def configure_logging() -> None:
    ...
    File handler:    DEBUG  → {LOG_DIR}/switchboard.log (10 MB × 5 backups)
    Console handler: INFO   → stdout (for journalctl / systemd)

    The 'switchboard' logger is set to DEBUG so all messages reach the file
    ...
    log_file = os.path.join(log_dir, "switchboard.log")

    try:
        os.makedirs(log_dir, exist_ok=True)
    except OSError as e:
        print(
            f"[switchboard] Warning: cannot create log directory {log_dir!r}: {e}",
            file=sys.stderr,
        )
        log_file = None
