...
class TestResumeResetsGateStatus:
    ...
    async def test_resume_preserves_gate_status(self, db, sample_project):
        ...
        # Use needs-review status (valid for resume via stopped effective state);
        ...
        assert updated["gate_status"] == "test-failed", (
            f"Expected gate_status='test-failed' preserved after resume, got {updated['gate_status']!r}"
        )
