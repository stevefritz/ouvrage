...
class TestWebPushDispatch:
    ...
    def _enable_vapid(self, monkeypatch):
        ...
        import switchboard.notifications.web_push as web_push
    ...
    async def test_failed_triggers_notification(self, mock_settings, one_subscription):
        with patch("switchboard.notifications.web_push.db.get_notification_settings", AsyncMock(return_value=mock_settings)), \
             patch("switchboard.notifications.web_push.db.get_push_subscriptions", AsyncMock(return_value=one_subscription)), \
             patch("switchboard.notifications.web_push._send_one", return_value=True) as mock_send:
            import switchboard.notifications.web_push as web_push
    ...
    async def test_no_subscriptions_no_crash(self, mock_settings):
        ...
        with patch("switchboard.notifications.web_push.db.get_notification_settings", AsyncMock(return_value=mock_settings)), \
             patch("switchboard.notifications.web_push.db.get_push_subscriptions", AsyncMock(return_value=[])):
            import switchboard.notifications.web_push as web_push
    ...
    async def test_send_failure_does_not_raise(self, mock_settings, one_subscription):
        ...
        with patch("switchboard.notifications.web_push.db.get_notification_settings", AsyncMock(return_value=mock_settings)), \
             patch("switchboard.notifications.web_push.db.get_push_subscriptions", AsyncMock(return_value=one_subscription)), \
             patch("switchboard.notifications.web_push._send_one", return_value=False):
            import switchboard.notifications.web_push as web_push
            # Should not raise
    ...
    async def test_settings_error_does_not_raise(self, one_subscription):
        ...
        with patch("switchboard.notifications.web_push.db.get_notification_settings", AsyncMock(side_effect=Exception("DB error"))), \
             patch("switchboard.notifications.web_push.db.get_push_subscriptions", AsyncMock(return_value=one_subscription)), \
             patch("switchboard.notifications.web_push._send_one", return_value=True) as mock_send:
            import switchboard.notifications.web_push as web_push
            await web_push.dispatch_notification("failed", "proj/task-a", "Failed", "Error")
    ...
    async def test_multiple_subscriptions_all_receive(self, mock_settings):
        ...
        with patch("switchboard.notifications.web_push.db.get_notification_settings", AsyncMock(return_value=mock_settings)), \
             patch("switchboard.notifications.web_push.db.get_push_subscriptions", AsyncMock(return_value=subs)), \
             patch("switchboard.notifications.web_push._send_one", return_value=True) as mock_send:
            import switchboard.notifications.web_push as web_push
