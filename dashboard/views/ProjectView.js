...
// ---------------------------------------------------------------------------
// ProjectSettingsSection — PAT override and config editing
// ---------------------------------------------------------------------------

function ProjectSettingsSection({ project, onUpdated }) {
    ...
    const hasProjectPat = !!(project && project.github_pat_override);
    ...
    const handleSavePat = async () => {
        ...
        setSuccess(false);
        try {
            const result = await api.updateProject(project.id, { github_pat_override: patValue });
            setPatValue('');
            setSuccess(true);
    ...
    const handleClearPat = async () => {
        ...
        setSuccess(false);
        try {
            const result = await api.updateProject(project.id, { github_pat_override: '' });
            setSuccess(true);
            if (onUpdated) onUpdated(result);
            setTimeout(() => setSuccess(false), 3000);
        } catch (err) {
    ...
    const statusBadgeStyle = {
        ...
        fontSize: typography.size.xs,
        fontWeight: typography.weight.medium,
        background: hasProjectPat ? colors.greenBg : colors.surface,
        color: hasProjectPat ? colors.green : colors.textTertiary,
        border: `1px solid ${hasProjectPat ? colors.green + '44' : colors.border}`,
    };
    ...
    return html`
        <div style=${sectionStyle}>
            <button type="button" style=${headerBtnStyle} onClick=${() => setOpen(o => !o)}>
                <span>${open ? '▾' : '▸'}</span>
                ...
                <div style=${bodyStyle}>
                    ...
                    <div>
                        <div style=${{ display: 'flex', alignItems: 'center', gap: '10px', marginBottom: '10px' }}>
                            <span style=${{ fontSize: typography.size.sm, fontWeight: typography.weight.medium, color: colors.textSecondary }}>GitHub PAT</span>
                            <span style=${statusBadgeStyle}>
                                ${hasProjectPat ? 'Using project PAT' : 'Using instance PAT (default)'}
                            </span>
                        </div>
                        <div style=${rowStyle}>
                            <input
                                type="password"
                                value=${patValue}
                                onInput=${e => setPatValue(e.target.value)}
                                style=${{ ...inputStyle, flex: 1 }}
                                placeholder=${hasProjectPat ? 'Enter new PAT to replace…' : 'ghp_…'}
                            ...
                            ${hasProjectPat ? html`
                                <button
                                    ...
                                    title="Remove project PAT — falls back to instance PAT"
                                >Clear</button>
                            ` : null}
                        </div>
                        <div style=${hintStyle}>Optional. Uses instance PAT if not set. Use this for repos that need a different token (different org, client repo, SaaS isolation).</div>
                        ${error ? html`<div style=${{ color: colors.red, fontSize: typography.size.xs, marginTop: '6px' }}>${error}</div>` : null}
                        ${success ? html`<div style=${{ color: colors.green, fontSize: typography.size.xs, marginTop: '6px' }}>Saved.</div>` : null}
                    </div>
                </div>
            ` : null}
        </div>
...
export function ProjectView({ id }) {
    ...
    return html`
        <div style=${pageStyle}>
            ...
            <!-- Project Settings (PAT override, etc.) -->
