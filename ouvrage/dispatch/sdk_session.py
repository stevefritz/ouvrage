"""ouvrage.dispatch.sdk_session — Claude Agent SDK session management.
...
from ouvrage.config.constants import MESSAGE_POLL_INTERVAL, DEFAULT_MODEL
...
async def _build_task_prompt(project: dict, task: dict, spec_content: str | None,
    ...
    if checklist:
        ...
        parts.append("Mark items done as you complete them using `mcp__ouvrage__update_task_checklist`.")
        parts.append("")
    ...
    if not review_feedback:
        ...
        parts.append("1. `ouvrage/server/handlers/tasks.py` — Add `files` array to the task status response.")
        parts.append("   Query `db.list_files(task_id)` in `_handle_get_task_status` and include compact file metadata.")
        parts.append("2. `ouvrage/server/tools.py` — Register `get_file` tool on user endpoint alongside existing `get_attached_file`.")
    ...
    parts.append(
        f"  `mcp__ouvrage__add_task_file(task_id='{task_id}', source_path='/absolute/path/in/worktree/file.pdf')`"
    )
    ...
    parts.append("Key tools: `post_task_message` (progress/question/result/handoff/plan), "
                 ...
                 "`git_push`, `git_fetch`.")
    parts.append("")
    parts.append(f"- Update checklist: `mcp__ouvrage__update_task_checklist(item_id=<id>, done=true)`")
    parts.append(f"- Update phase: `mcp__ouvrage__update_task_phase(task_id='{task_id}', phase='implementing', detail='...')`")
    parts.append(f"- Post progress: `mcp__ouvrage__post_task_message(task_id='{task_id}', author='cc-worker', type='progress', content='...')`")
    parts.append("")
    ...
    parts.append(f"3. Push your branch: `mcp__ouvrage__git_push(task_id='{task_id}')`")
...
async def _build_resume_prompt(task_id: str) -> str:
    ...
    if not task:
        return (
            ...
            f"Run `mcp__ouvrage__read_task_messages(task_id='{task_id}')` for any new instructions, "
            f"then continue."
        )
    ...
    parts.append(f"Check task messages for any new instructions posted while you were paused: `mcp__ouvrage__read_task_messages(task_id='{task_id}')`")
...
async def _setup_log_dir(worktree_path: str, clean: bool = True) -> Path:
    """Create .ouvrage log directory in the worktree.
    ...
    Also ensures .ouvrage is gitignored and removes any stale
    git-tracked .ouvrage files (which cause permission issues
    when inherited from parent branches).
    """
    log_dir = Path(worktree_path) / ".ouvrage"

    # If .ouvrage exists and is git-tracked, remove tracked files first
    # (they'll have wrong ownership from git checkout)
    stdout, _, rc = await _run_as_worker(
        "git", "-C", worktree_path, "ls-files", ".ouvrage",
    )
    if rc == 0 and stdout.strip():
        log.debug(f"Removing git-tracked .ouvrage files from {worktree_path}")
        await _run_as_worker("git", "-C", worktree_path, "rm", "-rf", "--cached", ".ouvrage")

    # Ensure .ouvrage is gitignored so CC never commits it
    gitignore_path = Path(worktree_path) / ".gitignore"
    if gitignore_path.exists():
        content = gitignore_path.read_text()
        if ".ouvrage" not in content:
            await _run_as_worker("sh", "-c", f"echo '.ouvrage/' >> {gitignore_path}")
    else:
        await _run_as_worker("sh", "-c", f"echo '.ouvrage/' > {gitignore_path}")
...
async def _run_sdk_session(
    ...
    mcp_servers = {
        "ouvrage": {
            "type": "http",
            "url": f"http://localhost:{os.environ.get('OUVRAGE_PORT', '8100')}/mcp/worker",
        },
    }
    try:
