"""ouvrage.dispatch.internals — status-agnostic dispatch building blocks.
...
async def setup_hook_config(worktree_path: str) -> None:
    ...
    The hook scripts live at /opt/ouvrage/hooks/ on the host —
            ...
            "PreToolUse": [
                {
                    ...
                    "hooks": [
                        {
                            "type": "command",
                            "command": "/opt/ouvrage/hooks/block-git-push.sh",
                        },
                        {
                            "type": "command",
...
async def collect_reopen_feedback(task_id: str, current_attempt: int) -> list[dict] | None:
    ...
    if reopen_msg_idx is not None:
        feedback = [
            ...
            if m.get("author") not in ("switchboard", "ouvrage", "dispatcher", "cc-worker")
        ]
        if feedback:
            return feedback
