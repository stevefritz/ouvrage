"""ouvrage.dispatch.engine — task lifecycle orchestration hub.
...
from ouvrage.config.constants import DEFAULT_MODEL
...
from ouvrage.dispatch.sdk_session import (
    _build_task_prompt,
    _setup_log_dir,
    _write_dispatch_log,
    _run_sdk_session,
...
async def archive_task_logs(task: dict, project: dict, reason: str) -> Path | None:
    """Copy .ouvrage/ contents to persistent .task-history archive.
    ...
    No-op if worktree is absent or .ouvrage/ doesn't exist.
    """
    worktree = task.get("worktree_path")
    if not worktree:
        return None

    src = Path(worktree) / ".ouvrage"
    if not src.exists():
        return None
...
async def dispatch_task(
    ...
    from ouvrage.config.constants import SYSTEM_DEFAULTS
