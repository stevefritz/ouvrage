...
from ouvrage.db.audit import write_audit_log
...
async def _stop_cc_session(task: dict, **ctx: Any) -> None:
    ...
    from ouvrage.dispatch._state import _running_tasks, _active_clients
    ...
    if worktree:
        log_path = Path(worktree) / ".ouvrage" / "session.jsonl"
        if log_path.exists():
...
async def _dispatch_launch_session(task: dict, **ctx: Any) -> None:
    ...
    import os
    from ouvrage.dispatch.internals import (
...
async def _resume_launch_session(task: dict, **ctx: Any) -> None:
    ...
    # Ensure hook config is present — idempotent, writes if missing.
...
async def _retry_launch_session(task: dict, **ctx: Any) -> None:
    ...
    from ouvrage.dispatch.internals import (
        ...
        collect_review_feedback, setup_hook_config,
    )
    from ouvrage.dispatch.engine import (
...
async def _start_launch_session(task: dict, **ctx: Any) -> None:
    ...
    from ouvrage.dispatch.internals import (
        ...
        launch_sdk_session, resolve_session_config,
        collect_reopen_feedback, setup_hook_config,
    )
...
async def _on_sdk_complete(task: dict, **ctx: Any) -> None:
    ...
    from ouvrage.dispatch.engine import _update_usage, _check_and_dispatch_dependents
    from ouvrage.dispatch.gates import _run_test_gate, _dispatch_review
    ...
    if task.get("parent_task_id"):
        from ouvrage.dispatch.gates import _process_review_result
