"""ouvrage.dispatch.recovery — crash recovery and stall detection.
...
from ouvrage.config.constants import STALL_THRESHOLD_SECONDS, STALL_CHECK_INTERVAL
...
async def mark_working_for_recovery():
    ...
    """
    from ouvrage.dispatch._state import _running_gates  # noqa: PLC0415
...
async def recover_orphaned_tasks():
    ...
    from ouvrage.dispatch.gates import _resume_gate_pipeline  # noqa: PLC0415
    from ouvrage.dispatch.lifecycle import lifecycle  # noqa: PLC0415
    ...
    for status in ("completed", "pending-validation", "turns-exhausted", "validating"):
        ...
        for task in stuck_tasks:
            ...
            if task.get("gate_passed_at"):
                ...
                if gate == "passed":
                    from ouvrage.dispatch.engine import _check_and_dispatch_dependents  # noqa: PLC0415
            ...
            if gate is not None or (status == "pending-validation" and gate is None):
                ...
                if handled is False:
                    ...
                    if refreshed and refreshed.get("gate_status") in ("test-failed", "review-failed"):
                        from ouvrage.dispatch.engine import retry_task  # noqa: PLC0415
...
async def _recover_gate_subtask(task_id: str, task: dict) -> None:
    ...
    from ouvrage.dispatch.gates import _run_test_gate, _dispatch_review  # noqa: PLC0415
    from ouvrage.dispatch.lifecycle import lifecycle  # noqa: PLC0415
...
async def _recover_with_resume(task_id: str, task: dict) -> None:
    ...
    from ouvrage.dispatch.engine import resume_task  # noqa: PLC0415
...
async def _recover_single_task(task: dict):
    ...
    # Lazy imports to break circular dependency
    from ouvrage.dispatch.engine import resume_task, retry_task  # noqa: PLC0415
    from ouvrage.dispatch.lifecycle import lifecycle  # noqa: PLC0415
...
async def check_stalled_tasks():
    ...
    # Lazy imports to break circular dependency
    from ouvrage.dispatch.engine import retry_task  # noqa: PLC0415
    from ouvrage.dispatch._state import _active_clients, _running_gates
    from ouvrage.dispatch.gates import _resume_gate_pipeline  # noqa: PLC0415
