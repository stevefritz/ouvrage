...
from ouvrage.auth import middleware as auth
from ouvrage.auth import oauth as oauth_server
from ouvrage.auth import sessions as session_server
from ouvrage.auth import sso as sso_server
from ouvrage.dashboard import api as dashboard_api
...
import ouvrage.dispatch as tasks

from ouvrage.server.tools import TOOLS, WORKER_TOOLS, WORKER_TOOL_ALLOWLIST
from ouvrage.server.dispatch import _dispatch_tool
from ouvrage.server.context import set_request_context, get_request_is_worker
from ouvrage.config.settings import OAUTH_BASE_URL
from ouvrage.config.nudges import inject_nudge
...
SERVER_INSTRUCTIONS = """\
Ouvrage is a task orchestration system. It dispatches autonomous Claude Code \
...
CONVERSATIONS ARE YOUR MEMORY: Ouvrage conversations persist across sessions. \
...
server = Server("ouvrage", instructions=SERVER_INSTRUCTIONS)
...
async def _serve_ouvrage(scope, send):
    """Serve index.html and its assets from dashboard/."""
    path = scope["path"]
    # /dashboard, /dashboard/, /dashboard/login → index.html at app root
    ...
    if path in ("/dashboard", "/dashboard/", "/dashboard/login"):
        file_path = os.path.join(_project_root, "index.html")
    else:
...
async def main():
    ...
    if _owner_email and _owner_hash:
        ...
        if not _existing:
            ...
            await _run_migrate_auth(
                email=_owner_email,
                name=_os.environ.get("OUVRAGE_OWNER_NAME", "Owner"),
                password_hash=_owner_hash,
                slug=_os.environ.get("OUVRAGE_INSTANCE_SLUG", "default"),
                instance_name=_os.environ.get("OUVRAGE_INSTANCE_NAME", "Ouvrage"),
            )
    ...
    async def app(scope, receive, send):
        ...
        if path == "/health" and method == "GET":
            ...
            await send({"type": "http.response.body", "body": b"Ouvrage OK"})
        elif path == "/mcp":
            ...
            await _serve_ouvrage(scope, send)
    ...
    port = int(os.environ.get("OUVRAGE_PORT", "8100"))

    from ouvrage.auth.middleware import _is_self_issuer
    if _is_self_issuer():
        from ouvrage.auth.middleware import _get_self_base_url
