...
from ouvrage.server.handlers.common import _embed_message_async, PR_URL_RE
from ouvrage.server.context import get_request_user_id, get_request_is_token_auth, get_request_is_worker, get_request_base_url
from ouvrage.config.settings import SKIP_CREDENTIAL_CHECK
...
async def _handle_transition_task(arguments):
    from ouvrage.dispatch.lifecycle import lifecycle, IllegalTransition
...
async def _handle_get_task_status(arguments):
    ...
    if include_detail:
        ...
        if arguments.get("include_log_tail") and result.get("worktree_path"):
            log_path = os.path.join(result["worktree_path"], ".ouvrage", "cc-stderr.log")
...
async def _handle_read_task_messages(arguments):
    ...
    if arguments.get("summary"):
        from ouvrage.server.handlers.conversations import _summarize_messages
...
def _resolve_log_dir(task: dict, project: dict | None, attempt: int | None) -> tuple[str | None, str | None]:
    ...
    if worktree and os.path.isdir(os.path.join(worktree, ".ouvrage")):
        return os.path.join(worktree, ".ouvrage"), "live"
