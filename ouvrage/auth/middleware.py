...
logger = logging.getLogger("ouvrage.auth")
...
def _get_local_jwks() -> dict[str, Any]:
    ...
    from ouvrage.auth.oauth import get_jwks
    return get_jwks()


def _get_self_base_url() -> str:
    """Return the expected issuer URL for self-issued tokens."""
    from ouvrage.auth.oauth import _get_base_url
...
def _get_instance_url(scope: dict) -> str:
    ...
    Returns e.g. 'https://tenant.ouvrage.dev'. Falls back to an empty string
