...
Usage:
    python -m ouvrage migrate-auth --email X --name Y --password-hash Z --slug W

Auto-migration on startup: set OUVRAGE_OWNER_EMAIL + OUVRAGE_OWNER_PASSWORD_HASH env vars.
"""
...
async def run_migrate_auth(
    ...
    """
    from ouvrage.db import init_db
    ...
    async with get_db() as db:
        ...
        await db.execute(
            """UPDATE messages SET user_id = ?
               WHERE author NOT IN ('dispatcher', 'cc-worker', 'switchboard', 'ouvrage')
               AND user_id IS NULL""",
            (owner_id,),
        )
        await db.commit()
    ...
    if rows:
        ...
        try:
            ...
            client_secret = "(could not decrypt — check OUVRAGE_MASTER_KEY)"
