"""Dashboard REST API — JSON endpoints for the Ouvrage SPA."""
...
from ouvrage.auth.oauth import get_client as _get_oauth_client
from ouvrage.config.constants import DEFAULT_MAX_CONCURRENT
from ouvrage.config import settings as _settings
from ouvrage.crypto import decrypt_value, encrypt_value, is_fernet_token
from ouvrage.git.operations import normalize_repo_url
from ouvrage.notifications import web_push
...
async def _resolve_dashboard_log_dir(task: dict, attempt: int | None):
    """Return a Path to the .ouvrage/ dir to read logs from.
...
async def _handle_test_output(scope, send, task_id):
    """Serve the live test output log file (.ouvrage/test-output.log)."""
    task = await db.get_task(task_id)
    if not task:
        return await _error(send, f"Task '{task_id}' not found", 404)
...
async def _handle_search_conversation(scope, send, conv_id: str):
    ...
    from ouvrage.embeddings.service import _get_openai_api_key
